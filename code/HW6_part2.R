...
Data = read.csv(file.path("data", "abalone_clean.csv"))
head(Data)
...
ggsave(file.path("graphs", "Whole.weight vs Height.png"))
...
ggsave(file.path("graphs", "Length vs Height.png"))
...
ggsave(file.path("graphs", "Length vs Diameter.png"))
...
ggsave(file.path("graphs", "Volume vs Whole.weight.png"))
...
ggsave(file.path("graphs", "Density of Rings of each Sex.png"))
...
ggsave(file.path("graphs", "Whole.weight vs Age for different Sex.png"))
...
ggsave(file.path("graphs", "Classification.png"))
...
#ggsave(file.path("graphs", "Predicted Probability.png"))
...
ggsave(file.path("graphs", "Classification using Whole.weight & Volume.png"))
ggplot(newdataL3, aes(x = Pred_Age3, y=Age, colour = factor(Pred_Age3==Age))) +
  geom_point(position = "jitter") + 
  scale_colour_manual("Age Group", values = c("2", "3"), labels = c("Wong Classification","Right Classification")) +
  ggtitle("Classification Using Volume")
ggsave(file.path("graphs", "Classification Using Volume.png"))
...
ggsave(file.path("graphs", "Min vs Max within each Observed Varialbes.png"))
...
ggsave(file.path("graphs", "ggplot vs xyplot.png"))
...
## JB: I'm pretty sure this is poorly written code. Loops should be rare in R. And there should never be rbind() or cbind() inside a loop. This uses tons of memory and is very inefficient. Not sure what you are doing / why, so I'm not re-writing this.
jFun <- function(x) {
  name = c()
  Coefs = c()
  for(i in 1:4 ){
...
ggsave(file.path("graphs", "Max_slope vs Min_slope of Height.png"))
...
ggplot(subset(Data, Sex %in% c("F","I")), aes(x = Rings, y = Diameter)) +   facet_wrap(~Sex, ncol=2) + 
  ...
  scale_colour_manual("Age Group",values = c("4","3","5"), labels = c("Young","Adult","Old")) 
ggsave(file.path("graphs", "Max_slope vs Min_slope of Diameter.png"))
...
ggplot(subset(Data, Sex %in% c("F","I")), aes(x = Rings, y = Length)) +   facet_wrap(~Sex, ncol=2) + 
  ...
  scale_colour_manual("Age Group",values = c("4","3","5"), labels = c("Young","Adult","Old")) 
ggsave(file.path("graphs", "Max_slope vs Min_slope of Length.png"))
...
ggplot(subset(Data, Sex %in% c("F","I")), aes(x = Rings, y = Whole.weight)) +   facet_wrap(~Sex, ncol=2) + 
  ...
  scale_colour_manual("Age Group",values = c("4","3","5"), labels = c("Young","Adult","Old")) 
ggsave(file.path("graphs", "Max_slope vs Min_slope of Whole.weight.png"))
